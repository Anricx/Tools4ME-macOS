...
class JSONViewController: AbstractTabItemViewController {
    ...
    @IBAction func onCompact(_ sender: Any) {
        ...
        do {
            let json = try JSONSerialization.jsonObject(with: text.data(using: String.Encoding.utf8)!, options: [])
            let data : Data = try JSONSerialization.data(withJSONObject: json, options: [])
            let payload : String = String(data : data, encoding : String.Encoding.utf8)!
            ...
            hud.labelText = NSLocalizedString("message.input.invalid.json", comment: "message.input.invalid.json")
